...
var configSchema = exports_external.object({
  basemailApiUrl: exports_external.string().default("https://api.basemail.ai"),
  geminiModel: exports_external.string().default("gemini-2.0-flash"),
  attestationContract: exports_external.string().default("0x60763E421030Ec629B25a0f22f40E2cDEB68490e"),
  chainSelectorName: exports_external.string().default("ethereum-testnet-sepolia-base-1")
});
var emailRequestSchema = exports_external.object({
  from: exports_external.string(),
  to: exports_external.string(),
  subject: exports_external.string(),
  body: exports_external.string(),
  sender_token: exports_external.string()
});
var QAF_BASE = 3;
var QAF_CAP = 10;
function qafPrice(unreadStreak) {
  if (unreadStreak <= 0)
    return QAF_BASE;
  const n = Math.min(unreadStreak + 1, QAF_CAP);
  return n * n;
}
var LLM_COEFFICIENTS = {
  spam: 3,
  cold: 1,
  legit: 0.5,
  high_value: 0.3,
...
var onHTTPTrigger = async (runtime2, payload, httpClient) => {
  runtime2.log("\uD83E\uDD9E The Diplomat — CRE Workflow triggered");
  ...
  runtime2.log(`\uD83D\uDCE7 Email: ${emailReq.from} → ${emailReq.to}: "${emailReq.subject}"`);
  const apiUrl = runtime2.config.basemailApiUrl;
  let geminiKey = "";
  try {
    const secret = runtime2.getSecret({ id: "GEMINI_API_KEY" }).result();
    geminiKey = secret.value;
  } catch (e) {
    runtime2.log("   ⚠️ Could not fetch GEMINI_API_KEY secret, LLM will use fallback");
  }
  runtime2.log("\uD83D\uDCCA Step 1: Querying QAF history...");
  const historyResp = httpClient.sendRequest(runtime2, {
    url: `${apiUrl}/api/diplomat/history?from=${emailReq.from}&to=${emailReq.to}`,
    method: "GET",
    headers: {}
  });
  const history = json(historyResp.result());
  runtime2.log(`   Unread streak: ${history.unread_streak}, QAF multiplier: ${history.qaf.multiplier}`);
  runtime2.log("\uD83E\uDD16 Step 2: LLM Arbitration (Gemini)...");
...
{"category": "spam|cold|legit|high_value|reply", "score": 0-10, "reasoning": "brief explanation"}`;
  let llmResult;
  try {
    const geminiResp = httpClient.sendRequest(runtime2, {
      url: `https://generativelanguage.googleapis.com/v1beta/models/${runtime2.config.geminiModel}:generateContent?key=${geminiKey}`,
      method: "POST",
    ...
    const geminiData = json(geminiResp.result());
    const text = geminiData.candidates?.[0]?.content?.parts?.[0]?.text || "{}";
    llmResult = JSON.parse(text);
  } catch (e) {
    runtime2.log('   LLM fallback: defaulting to "cold"');
    llmResult = { category: "cold", score: 5, reasoning: "LLM unavailable, default classification" };
  }
  runtime2.log(`   Category: ${llmResult.category}, Score: ${llmResult.score}/10`);
  runtime2.log(`   Reasoning: ${llmResult.reasoning}`);
  runtime2.log("\uD83D\uDCB0 Step 3: Calculating price...");
  ...
  runtime2.log(`   QAF base: ${qafBase} (n=${history.unread_streak})`);
  runtime2.log(`   LLM coefficient: ${llmCoeff} (${llmResult.category})`);
  runtime2.log(`   Final cost: ${finalCost} ATTN`);
  runtime2.log("\uD83D\uDCE4 Step 4: Sending email with Diplomat pricing...");
  const sendResp = httpClient.sendRequest(runtime2, {
  ...
  const sendResult = json(sendResp.result());
  if (!sendResult.success) {
    throw new Error(`Failed to send: ${sendResult.error || "Unknown error"}`);
  }
  runtime2.log(`   ✅ Email sent! ID: ${sendResult.email_id}`);
  runtime2.log(`   ATTN staked: ${finalCost}`);
  runtime2.log("\uD83D\uDCDD Step 5: On-chain attestation");
  runtime2.log(`   Contract: ${runtime2.config.attestationContract} (Base Sepolia)`);
  ...
  runtime2.log(`\uD83E\uDD9E Diplomat complete: ${summary}`);
  return summary;
};
var initWorkflow = (config) => {
  ...
  return [
    cre.handler(httpTrigger.trigger({}), (runtime2, payload) => onHTTPTrigger(runtime2, payload, httpClient))
  ];
};
async function main() {
